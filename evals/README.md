...
어느 부품을 개선할지 고르기 위해 시도마다 `cause`, 과제마다 `failure_cause`를 기록합니다. 모델의 답변 문장은 보지 않고 판정과 수정된 파일만으로 정합니다. 기준은 정답 패치(`solution.edits`)가 고치는 파일입니다.

| 원인 | 조건 | 개선 대상 |
|---|---|---|
| `LOCALIZATION` | 수정한 파일이 모두 정답 패치 밖인데 적용 실패·테스트 실패 | 문맥 검색·LSP·문맥 패커 |
| `FORMAT` | 형식 오류, 잘림, 맞는 파일인데 SEARCH 불일치 | 출력 형식·구조화 출력·프롬프트 |
| `LOGIC` | 정답 파일을 고쳤지만 필요한 테스트 실패 또는 회귀 | 모델·실패 피드백·다중 후보 |
| `TEST_ENVIRONMENT` | 러너 환경 문제, 사용할 수 없는 정답 패치 | 러너·과제 |
| `MODEL_CALL` | 모델 호출 실패 | 서버·설정 |
| `UNDETERMINED` | 테스트 시간 초과, 시도 없음 | 추가 근거 필요 |

정답 패치는 가능한 수정 중 하나이므로, 다른 파일을 고쳐 테스트를 통과하면 `PASS`이고 원인은 기록하지 않습니다.
